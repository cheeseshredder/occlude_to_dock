"""
QMDP Navigation Demo - HEAVY OCCLUSION TEST

Tests TRUE QMDP under heavy occlusion where charger is initially HIDDEN.
Robot starts at (0,0) directly behind plant - must explore to find charger.

This is the harder test case!
"""
...
def main():
    print("="*60)
    print("TRUE QMDP NAVIGATION - HEAVY OCCLUSION")
    print("="*60)
    print("Robot starts at (0,0) - charger completely hidden by plant")
    print("This tests exploration capability!")
    print("="*60)
    
    output_dir = os.path.join(project_root, "qmdp_demo_heavy")
    ...
    try:
        ...
        print("\n[1/7] Setting up HEAVY OCCLUSION scene...")
        # KEY CHANGE: 'heavy' instead of 'partial'
        ...
        print("\n[6/7] Creating TRUE QMDP planner...")
        ...
        planner = QMDPPlannerReal(
            ...
            reward_step=-1.0,
            lambda_risk=0.5,
            lambda_ig=2.0,  # Higher exploration bonus for heavy occlusion
        ...
        def save_visualization(step, mode, robot_pose, detections, target=None, rgb_frame=None):
            ...
            draw.text((430, 325), f"Step {step} | QMDP: {mode} | HEAVY OCCLUSION", fill=(0, 0, 0))
            ...
            if charger_pos:
                ...
                draw.text((430, 405), f"Charger: NOT DETECTED", fill=(150, 0, 0))
            
            if target:
                draw.text((430, 440), f"Target: ({target[0]:.1f}, {target[1]:.1f})", fill=(0, 0, 150))
            
            draw.text((10, 460), "TRUE QMDP - HEAVY OCCLUSION TEST", fill=(128, 0, 0))
        ...
        print("\n" + "="*60)
        print("STARTING TRUE QMDP NAVIGATION - HEAVY OCCLUSION")
        print("="*60)
        ...
        print("  ⚠️ Charger should NOT be visible from this position!")
        ...
        if not charger_visible:
            print("  ✅ CORRECT: Charger is hidden (heavy occlusion working)")
        else:
            print("  ⚠️ WARNING: Charger visible - occlusion may not be complete")
        ...
        max_steps = 30  # More steps for heavy occlusion
        step = 0
        success = False
        total_sim_steps = 0
        ...
        while step < max_steps:
            ...
            for det in detections:
                ...
                print(f"  {marker} {det['class']}: conf={det['confidence']:.2f}, dist={det['estimated_distance']:.2f}m")
        ...
        print("\n" + "="*60)
        print("FINAL RESULTS - TRUE QMDP - HEAVY OCCLUSION")
        print("="*60)
        ...
        if docking_success:
            print("\n🎉 MISSION SUCCESS - TRUE QMDP DOCKED UNDER HEAVY OCCLUSION!")
        elif success:
            print(f"\n⚠️ Docking attempted but distance {edge_to_face_dist:.2f}m > threshold")
        else:
            print("\n❌ Mission incomplete - did not reach DOCK state")
